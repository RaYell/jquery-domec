...
$(document).ready(function () {
	...
	test("Attributes", function () {
		...
		ok(elem.attr('id') === "myId", "(Multiple attributes) We expect element's identifier of myId");
		ok(elem.attr('name') === "myName", "(Multiple attributes) We expect element's name of myName");
		elem = $.create('div', null);
		ok(elem.attr('id') === "", "We expect null attributes parameter to be ignored");
		elem = $.create('div', ['id: myId']);
		ok(elem.attr('id') === "", "We expect malformed attributes parameter to be ignored (array)");
		elem = $.create('div', 'id: myId');
		ok(elem.attr('id') === "", "We expect malformed attributes parameter to be ignored (string)");
		elem = $.create('div', 10);
		ok(elem.attr('id') === "", "We expect malformed attributes parameter to be ignored (number)");
		elem = $.create('div', true);
		ok(elem.attr('id') === "", "We expect malformed attributes parameter to be ignored (boolean)");
	});
	...
	test("Children", function () {
		...
		ok(elem.children().get(0).tagName === "P", "(Single child) We expect element to have P child element");
		elem = $.create('div', null, [$.create('p')]);
		ok(elem.children().get(0).tagName === "P", "(Single child - array parameter) We expect element to have P child element");
		elem = $.create('div', null, [$.create('p'), $.create('span')]);
		ok(elem.children('p').get(0).tagName === "P", "(Multiple child) We expect element to have P child element");
		ok(elem.children('span').get(0).tagName === "SPAN", "(Multiple child) We expect element to have SPAN child element");
		...
		ok(elem.children().length === 0, "We expect malformed children parameter to be ignored (number)");
		elem = $.create('div', null, true);
		ok(elem.children().length === 0, "We expect malformed children parameter to be ignored (boolean)");
	});
